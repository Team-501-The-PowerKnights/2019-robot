...
public class ClimberFactory
{
   ...
   private static final Logger logger =
      RioLogger.getLogger( ClimberFactory.class.getName() );
   ...
   public static synchronized void constructInstance()
   {
      ...
      PropertiesManager propsMgr =
         new PropertiesManager( PropertyNames.Climber.name );
      boolean realSubsystem =
         propsMgr.getBoolean( PropertyNames.Climber.useRealSubsystem );

      if ( realSubsystem )
      {
         logger.info( "constructing real {} subsystem", myName );
         frc.robot.subsystems.impl.ClimberSubsystem.constructInstance();
         ourInstance = frc.robot.subsystems.impl.ClimberSubsystem.getInstance();
      }
      else
      {
         logger.warn( "constructing stub {} subsystem", myName );
         frc.robot.subsystems.stub.ClimberSubsystem.constructInstance();
         ourInstance = frc.robot.subsystems.stub.ClimberSubsystem.getInstance();
      }
