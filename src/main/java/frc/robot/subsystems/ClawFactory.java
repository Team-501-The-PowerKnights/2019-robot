...
public class ClawFactory
{
   ...
   private static final Logger logger =
      RioLogger.getLogger( ClawFactory.class.getName() );

   /** Singleton instance of class for all to use **/
   private static IClawSubsystem ourInstance;
   /** Name of our subsystem **/
   private static final String myName = SubsystemNames.clawName;
   ...
   public static synchronized void constructInstance()
   {
      ...
      PropertiesManager propsMgr =
         new PropertiesManager( PropertyNames.Claw.name );
      boolean realSubsystem =
         propsMgr.getBoolean( PropertyNames.Claw.useRealSubsystem );

      if ( realSubsystem )
      {
         logger.info( "constructing real {} subsystem", myName );
         frc.robot.subsystems.impl.ClawSubsystem.constructInstance();
         ourInstance = frc.robot.subsystems.impl.ClawSubsystem.getInstance();
      }
      else
      {
         logger.warn( "constructing stub {} subsystem", myName );
         frc.robot.subsystems.stub.ClawSubsystem.constructInstance();
         ourInstance = frc.robot.subsystems.stub.ClawSubsystem.getInstance();
      }
